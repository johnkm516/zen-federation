...
//This subgraph's name is set to "crm".
//Input type args is modified to be preceded by "crm_" because inputs are merged using the intersection strategy in the current version of Apollo Federation and directives are not supported with input types.
export const renamedInputTypesSchema = async (schema: GraphQLSchema) => {
  ...
  return wrapSchema({
    schema: schema,
    transforms: [
      new RenameTypes(name => (inputTypes.includes(name) ? `crm_${name}` : name)),
